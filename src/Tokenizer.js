...
class Tokenizer {
            ...
            for (let i = 0; i < l; i++) {
                ...
                if (i !== l - 1) {
                    ...
                    if (
                        isordered
                            ? b.length === 1 ||
                              (!isparen && b[b.length - 1] === ")")
                            : b.length > 1 ||
                              (this.options.smartLists && b !== bull)
                    ) {
                        addBack = itemMatch.slice(i + 1).join("\n");
                        list.raw = list.raw.substring(
                            0,
                            list.raw.length - addBack.length
                        );
                        i = l - 1;
                    }
                }
    ...
    strong(src, maskedSrc, prevChar = "") {
        ...
        if (
            ...
            while ((match = endReg.exec(maskedSrc)) != null) {
                cap = this.rules.inline.strong.middle.exec(
    ...
    em(src, maskedSrc, prevChar = "") {
        ...
        if (
            ...
            let cap;
            while ((match = endReg.exec(maskedSrc)) != null) {
                cap = this.rules.inline.em.middle.exec(
